...
public class main {
	public static void main(String[] args) throws IOException{
        ...
        Campsite campsite_obj = new Campsite("C:/Users/user/eclipse-workspace/FunProg2PProject/Accomodation.txt"  , "C:/Users/user/eclipse-workspace/FunProg2PProject/Clients.txt") ; 
	...
	public static void welcome_screen(int i , Campsite campsite_obj) {
		...
		try {
		 ...
		 if(a == 1) {
			...
			System.out.println("ERROR: entered character must be a number; ");
			welcome_screen(1 , campsite_obj)  ; 
		}
	}
	public static void Return_button(Customers client, Campsite campsite_obj) {
		System.out.println("if you want to go back to the OPTIONS (MENU) press *5* , if you want to go back to the MAIN MENU press *any other number*;"); 
		try {
		try (Scanner sc = new Scanner(System.in)) {
			...
			System.out.println("ERROR occured. Please enter a number; ") ; 
	...
	public static void working_screen(Customers client, Campsite campsite_obj) {
		...
		if( a== 3 ) {
			...
			try {
						...
						if(result== 2000) {
							System.out.println("ERROR: This accamodation doesnt exist.");
							Return_button(client,campsite_obj) ; 
						}
						else {
							if(campsite_obj.get_accomo(result).getifavailable()) {
								System.out.println("The chosen accomodation is AVAILABLE, can you enter the number of days you want to reserv it: ");
								try{
								...
								System.out.println("ERROR: sorry its not available at the moment" ); 
								Return_button(client,campsite_obj) ; 
							}
						}
			}catch(Exception e){
		...
		if( a == 7 ) {
			...
			System.out.println("Please choose one of the indicated options;") ;
			Return_button(client,campsite_obj) ;
		}
		}catch(Exception e) {
			System.out.println("ERROR: please correct your ID number format: 8 numbers and 1 letter;") ;
			Return_button(client,campsite_obj) ;
		}
	}
